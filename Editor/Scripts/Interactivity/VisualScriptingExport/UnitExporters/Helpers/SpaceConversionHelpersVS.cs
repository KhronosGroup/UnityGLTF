...
{
    public class SpaceConversionHelpersVS : SpaceConversionHelpers
    {
        #region Position Space Conversion
        
        public static void AddSpaceConversionWithCheck(UnitExporter unitExporter, ValueOutRef unitySpaceVector3, out ValueOutRef convertedVector3Socket)
        {
            if (!unitExporter.vsExportContext.plugin.addUnityToGltfSpaceConversion)
            {
                convertedVector3Socket = unitySpaceVector3;
                return;
            }
            AddSpaceConversion(unitExporter, unitySpaceVector3, out convertedVector3Socket);
        }
        
        public static void AddSpaceConversionWithCheck(UnitExporter unitExporter, ValueInput unitySpaceVector3, out ValueOutRef convertedVector3Socket)
        {
            if (!unitExporter.vsExportContext.plugin.addUnityToGltfSpaceConversion)
            {
        ...
        public static void AddRotationSpaceConversionWithCheck(UnitExporter unitExporter, ValueOutRef unitySpaceQuaternion, out ValueOutRef convertedQuaternion)
        {
            if (!unitExporter.vsExportContext.plugin.addUnityToGltfSpaceConversion)
            {
            ...
            AddRotationSpaceConversion(unitExporter, unitySpaceQuaternion, out convertedQuaternion);
        }
        
        public static void AddRotationSpaceConversionWithCheck(UnitExporter unitExporter, ValueInput unitySpaceQuaternion, out ValueOutRef convertedQuaternion)
        {
            if (!unitExporter.vsExportContext.plugin.addUnityToGltfSpaceConversion)
            {
                var tempNode = unitExporter.CreateNode(new Math_MulNode());
