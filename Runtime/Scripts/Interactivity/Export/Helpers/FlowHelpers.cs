...
namespace UnityGLTF.Interactivity.Export
{
    public class FlowHelpers
    {
            ...
            out FlowOutRef completed)
        {
            ...
            var ascendingCondition = exporter.CreateNode(new Math_LeNode());
            ...
            var addNode = exporter.CreateNode(new Math_AddNode());
            ...
            var ascendingIndexCondition = exporter.CreateNode(new Math_LtNode());
            ascendingIndexCondition.ValueIn("a").ConnectToSource(indexVarValue);
            endIndex = endIndex.Link(ascendingIndexCondition.ValueIn("b"));

            var descendingIndexCondition = exporter.CreateNode(new Math_GtNode());
            ...
            var conditionSelect = exporter.CreateNode(new Math_SelectNode());
            ...
            out FlowOutRef completed)
        {
            ...
            var whileNode = exporter.CreateNode(new Flow_WhileNode());
            var setStartIndexVar = VariablesHelpers.SetVariable(exporter, indexVar);
        ...
        {
            var setVarStart = exporter.CreateNode(new Variable_SetNode());
            var setVarFinish = exporter.CreateNode(new Variable_SetNode());
            var getVar = exporter.CreateNode(new Variable_GetNode());
            var varId = exporter.Context.AddVariableWithIdIfNeeded("waitWhile" + System.Guid.NewGuid(), false,
                GltfTypes.Bool);
            var tick = exporter.CreateNode(new Event_OnTickNode());
            var branch = exporter.CreateNode(new Flow_BranchNode());
            var waitingBranch = exporter.CreateNode(new Flow_BranchNode());
